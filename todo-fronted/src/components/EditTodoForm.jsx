...
const EditTodoForm = () => {
  ...
  useEffect(() => {
    if (!searchParams.get("id")) {
      ...
      if (auth.token) {
        axios
          .get(process.env.REACT_APP_API_BASE_URL + "/todo/" + searchParams.get("id"), {
            headers: {
  ...
  }, [searchParams, navigate, auth.token]);
  ...
  const handleSubmit = (e) => {
    ...
    if (todoTitle && todoDescription) {
      axios
        .put(process.env.REACT_APP_API_BASE_URL + "/todo/" + searchParams.get("id"), {
          title: todoTitle,

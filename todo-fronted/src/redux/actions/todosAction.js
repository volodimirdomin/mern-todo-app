...
export const getAllTodos = (payload) => async (dispatch) => {
  dispatch({ type: GET_TODOS_REQUEST });

  try {
    const res = await axios.get(process.env.REACT_APP_API_BASE_URL + "/todos-all", {
      headers: {
...
export const createTodo = (payload) => async (dispatch) => {
  ...
  try {
    const res = await axios.post(process.env.REACT_APP_API_BASE_URL + "/todo/new", {
...
export const deleteTodo = (payload) => async (dispatch) => {
  ...
  try {
    const res = await axios.delete(process.env.REACT_APP_API_BASE_URL + "/todo/" + payload.todoId, {
      headers: {
        Authorization: `Bearer ${payload.token}`,
      }
    });
